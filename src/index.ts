...
app.all('*', async (c) => {
	...
	return new HTMLRewriter().on(DetectGtm.selector, new DetectGtm(true)).on(GTMToPartytown.selector, new GTMToPartytown(detector.sctips)).transform(res)
})
...
class GTMToPartytown implements HTMLRewriterElementContentHandlers {
	...
	async element(element: Element): Promise<void> {
		...
		const partytownScript = await fetch('https://www.unpkg.com/@builder.io/partytown@0.8.1/lib/partytown.js', { cf: { cacheEverything: true } })
		element.prepend(`<script>${await partytownScript.text()}</script><script><script>partytown = { forward: ["dataLayer.push"] }</script></script>`, { html: true })
